...
*   **Yards Gained**: The coefficient is -0.001329. This suggests a very small negative impact of yards gained on wins, but this effect is not statistically significant (p = 0.34332), implying that as yards gained increase, the number of wins slightly decreases, which is counter intuitive and likely indicates no real effect or that the model isn't capturing the complexities.
...
As a whole the model has statistical significance. Though there are parts that are not stat significant, when used all together the information can be useful. For example, yards gain is not significant according to the p-value, but point scored is significant according to their p-value. 
...
Point scored has the most impact on wins with a correlation score of 0.7078293, and yards gained has the least impact with a score of 0.4430188. 
...
I would use this model to better understand how different factors of a game impact the winning results. Factors such as Yards Gained, Takeaways, Giveaways, Yard Allowed, Point Scored, and Wins are all used and input into the model to calculate their statistical significant and correlation matrix. Though it is best to specifically choose factors that are relevant to what I need to understand and use the model to test it. Inputting everything into the model could mess up the results and provide different outcomes. 
