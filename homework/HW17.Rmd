...
- According to the scatterplots, there is no clear correlation or relationship between the predictor and voter turnout. Some indepdent variables are a little more linear than others, like pct_unemployed and pct_white, but the other variables are either too spread out or congregated at one end, like population. 
- According to the correlation matrix, the one with a highest positive correlation is pct_white and lowest negative is pct_unemployed. The closest one to a middle ground is population. Since the correlation matrix has a positive and a negative, the data displays the voter turnout for each independent variable in positive percentages or negative percentages. With that in mind, pct_white would have a positive correlation to the voter turnout and pct_unemployed would have a negative. Population is the closest to 0.
...
I would say this is a useful model because the residual min and max range is not severe, meaning the range of differences are not far off, between -0.05 and 0.02. Compared to previous homeworker where the range was way into the double digits. Even the statistically insignificant estimates for the independent variables are useful for telling is and is not correlated to voter turnout. I had to google this but if the p-value is 0.000002999 then it is also statistically significant to the research. 
...
```{r}
# Model 2
model2 <- lm(turnout ~ population + income + pct_bachelors_higher + median_age + pct_white, data = md_turnout)
summary(model2)

# Model 3
model3 <- lm(turnout ~ median_age + pct_bachelors_higher + pct_unemployed + pct_white, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ population + pct_bachelors_higher + pct_white + pct_unemployed, data = md_turnout)
summary(model4)
...
Regarding the analysis of a model which uses statistical methods to understand the correlations between voter turnouts and population, income, education, percentage of unemployed, median age, and percentage of white population, the model shows the different outcomes when considering one independent variable at a time and all of them together and how they influence voter turnout. I think depending on the specific model used to display this information, for example between the correlation matrix, scatterplot, and model comparison adjusted by rsquared, researchers will interpret the importance of these independent variables differently. 

In all, used correctly and with a specific question in mind to answer, an election or voter participation story could be supported using this model. 
